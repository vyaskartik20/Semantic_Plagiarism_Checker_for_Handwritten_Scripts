...
def FleschReadingEase(text, NoOfsentences):
    ...
    I = 206.835 - 1.015 * (l / float(NoOfsentences)) - \
        84.6 * (scount / float(l))
    return I
...
def dale_chall_readability_formula(text, NoOfSectences):
    ...
    adjusted = 0
    NoOfWords = len(words)
    with open('dale-chall-unix.pkl', 'rb') as f:
...
def Analysis(vector, K=3):
    ...
    for i in range(len(components)):
        plt.plot(components[i][0], components[i][1],
                 colors[lables[i]], markersize=10)

    plt.scatter(centers[:, 0], centers[:, 1], marker="x",
                s=150, linewidths=10, zorder=15)

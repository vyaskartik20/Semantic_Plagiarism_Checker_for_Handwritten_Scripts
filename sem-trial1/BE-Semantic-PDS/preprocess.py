...
def generate_bigrams(text, save_filename='data/useful_bigrams.txt', read_percent=1, bigram_percent=0.05):
    ...
    >> [a_b, c_d, ]
    """
    stopwords = open("stopwords.txt", mode='r', encoding='utf8').read().splitlines()
...
class Preprocess:
    def __init__(self, bigram_filename="data/useful_bigrams.txt", bigrams=False, vocab_size=None,
        ...
        self.stopwords = open("stopwords.txt", mode='r', encoding='utf8').readlines()
...
if __name__ == '__main__':
    text = open("msr_paraphrase_data.txt", mode='r', encoding='utf8').read().lower()
